...
- **Sort keys (2026-08-17):** `midSortKey` uses `fractional-indexing` (rocicorp) for canonical short keys on new writes; tolerant `incTail`/`decTail` fallback for legacy non-canonical keys (`a20`/`a00`/`a200` — same position as `a2`/`a0`, library rejects them, was silently crashing outdent). All sibling sorts are byte-order (`compareRows`/`compareKeys`), never `localeCompare`. 10 unit tests in `packages/web/src/lib/sortkeys.test.ts`. Twos-style structural keys while editing: Space at caret-0 indents, Backspace-on-empty outdents (owner, saves in-progress text). Mobile indent `--indent` (1rem desktop / .7rem phone) + Logseq-style left guide line.
